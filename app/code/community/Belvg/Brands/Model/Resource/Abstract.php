 ...
 */
class Belvg_Brands_Model_Resource_Abstract extends Mage_Eav_Model_Entity_Abstract
{
    ...
    {
        return 'brands/resource_eav_attribute';
    }
     ...
     * @return Belvg_Brands_Model_Resource_Abstract
     */
            ...
            if (isset($this->_attributes[$valueRow['attribute_id']])) {
                if ($isDefaultStore) {
                    ...
                    $object->setAttributeDefaultValue(
                            $attributeCode, $this->_attributes[$valueRow['attribute_id']]['value']
                    );
                }
            } else {
    ...
    Mage_Eav_Model_Entity_Attribute_Abstract $attribute, $value, array &$origData)
    {
        ...
        if ($typedAttributes) {
            foreach ($typedAttributes as $table => $_attributes) {
                ...
                if ($store != $this->getDefaultStoreId()) {
                    $valueExpr = $adapter->getCheckSql('store_value.value IS NULL', 'default_value.value', 'store_value.value');
                    $joinCondition = array(
                    ...
                    $select->joinLeft(
                            array('store_value' => $table), implode(' AND ', $joinCondition), array('attr_value' => $valueExpr)
                    );
     ...
     * @return Mage_Catalog_Model_Resource_Abstract
     */
    public function load1($object, $entityId, $attributes = array())
    {
